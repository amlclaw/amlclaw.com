...
export async function POST(req: Request) {
  ...
  if (type === "address") {
    ...
    task = {
      ...
      address,
      tokens,
      min_amount: Number(body.min_amount ?? settings.monitoring.defaultMinAmount ?? 10),
      cursor,
      in_ruleset_id: parseInt(String(body.in_ruleset_id ?? settings.screening.defaultKytInRulesetId)) || 0,
      out_ruleset_id: parseInt(String(body.out_ruleset_id ?? settings.screening.defaultKytOutRulesetId)) || 0,
      schedule,
    ...
    task = {
      id,
      type,
      name: (body.name || "").trim() || `${chain} TX 监控 · ${watchSide === "from" ? "发送方" : "接收方"}`,
      chain,
      address,
      origin_tx_id: txId,
      watch_side: watchSide,
      kya_ruleset_id: parseInt(String(body.kya_ruleset_id ?? settings.screening.defaultKyaRulesetId)) || 0,
      last_risk_level: "low",
      schedule,
      schedule_preset: schedulePreset,
      enabled: body.enabled !== false,
